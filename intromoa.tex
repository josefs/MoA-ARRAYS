...
that theoretical anamolies were introduced. More on this later.
...
to define an algebra of arrays based on shapes; others followed\cite{lyon,guibas}. He believed that if array operations were based on 
...
It was  Alan Perlis  who realized that these optimizations should be done at compile time\cite{perlis}, others
followed\cite{miller,budd84}. Perlis was also the first to recognize that there were too many anamolies in both APL's
algebra and environment; he recommended a functional array calculator based on the lambda 
...
When compiler support was implemented in APL,
the main limitation was that the anomalies in the language~\cite{gerhart} 
made it difficult to optimize and impossible to verify correctness of 
programs.   

MoA and the Psi Calculus, as a theory  has  also been used to describe and verify 
...
of a processor, memory, communication topology. . 
%
%
%
%\bibliographystyle{plain}
%\bibliography{paper1,hpf,workshop}
\section*{Elements of the theory}
...
\hspace{2cm}\= $u \;\cat\; v$ \hspace{1cm} \=		\=catentation of vectors $u$ and $v$ \\
